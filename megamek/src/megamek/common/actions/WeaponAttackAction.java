...
public class WeaponAttackAction extends AbstractAttackAction implements
    ...
    private static ToHitData toHit(IGame game, int attackerId,
        ...
        boolean bArtemisV = ((mLinker != null)
                ...
                && !isECMAffected && !bMekTankStealthActive && (atype != null) && (atype
        ...
        if (isIndirect) {
            ...
            if ((spotter == null)
                    ...
                    && ((atype.getAmmoType() == AmmoType.T_LRM)
                            || (atype.getAmmoType() == AmmoType.T_MML)
                            || (atype.getAmmoType() == AmmoType.T_NLRM) || (atype
        ...
        // they can only target the Mek they're swarming.
            ...
            if (ae instanceof BattleArmor) {
                if (!Infantry.SWARM_WEAPON_MEK.equals(wtype.getInternalName())
                    ...
                    return new ToHitData(TargetRoll.IMPOSSIBLE,
                            "Use the 'Attack Swarmed Mek' attack instead");
                }
                return new ToHitData(TargetRoll.AUTOMATIC_SUCCESS,
                        "swarming (automatic hit)", ToHitData.HIT_SWARM, side);
            }
            return new ToHitData(TargetRoll.AUTOMATIC_SUCCESS, "swarming",
        ...
        if (!isArtilleryIndirect) {
            toHit.append(AbstractAttackAction.nightModifiers(game, target,
                    atype, ae, true));
        }
        ...
        if ((te != null) && targHex.containsTerrain(Terrains.WATER)
                ...
                && (targEl == 0) && (te.height() > 0)) { // target in partial
                                                         // water
        ...
        // target immobile
        boolean mekMortarMunitionsIgnoreImmobile = weapon.getType().hasFlag(
                WeaponType.F_MEK_MORTAR)
                && (atype != null)
        ...
        if (los.getTargetCover() != LosEffects.COVER_NONE) {
            if (underWater
                ...
                toHit.setDamagableCoverTypePrimary(los
                ...
                toHit.setDamagableCoverTypeSecondary(los
    ...
    private static String toHitIsImpossible(IGame game, Entity ae,
        ...
        if ((ae instanceof BattleArmor)
                && (weapon.getType().hasFlag(WeaponType.F_TASER) || wtype
                        .getAmmoType() == AmmoType.T_NARC)) {
        ...
        if ((target.getTargetType() == Targetable.TYPE_FLARE_DELIVER)
                && !(usesAmmo
                        && ((atype.getAmmoType() == AmmoType.T_LRM)
                                || (atype.getAmmoType() == AmmoType.T_MML) || (atype
                                .getAmmoType() == AmmoType.T_MEK_MORTAR)) && (atype
        ...
        if (wtype.hasModes()
                ...
                && (RangeType.rangeBracket(
                        ...
                        wtype.getRanges(weapon), true, false) == RangeType.RANGE_SHORT)) {
        ...
        if (wtype.hasFlag(WeaponType.F_SOLO_ATTACK)) {
            ...
            for (Enumeration<EntityAction> i = game.getActions(); i
                ...
                if (prevAttack.getEntityId() == attackerId) {
                    Mounted prevWeapon = ae.getEquipment(prevAttack
                            .getWeaponId());
                    if ((prevWeapon.getType().hasFlag(WeaponType.F_INFANTRY) && (weapon
                            .getLocation() == Infantry.LOC_FIELD_GUNS))
                            || (weapon.getType().hasFlag(WeaponType.F_INFANTRY) && (prevWeapon
                                    .getLocation() == Infantry.LOC_FIELD_GUNS))) {
        ...
        if ((target.getTargetType() == Targetable.TYPE_ENTITY)
            ...
            if (!ae.getPosition().translated(ae.getFacing(), distance)
        ...
        if (weapon.getType().hasFlag(WeaponType.F_MEK_MORTAR)
                && (atype != null)
                && ((atype.getMunitionType() == AmmoType.M_AIRBURST)
                        || (atype.getMunitionType() == AmmoType.M_FLARE) || (atype
                        .getMunitionType() == AmmoType.M_SMOKE))) {
        ...
        for (Enumeration<EntityAction> actions = game.getActions(); actions
            ...
            if (ea instanceof WeaponAttackAction) {
                ...
                if (waaAE.equals(attacker)) {
                    ...
                    // type, fail.
                    if (!waaAE.getEquipment(waa.getWeaponId()).getType()
